...
class MessagingManager {
  ...
  _fire_event_listeners (message) {
    ...
    for (const event_handler of this._event_handlers.get(message.action)) {
...
class InterfaceManager {
  ...
  _update_statistics (statistics) {
    ...
    for (const media_type in statistics) {
      ...
      if (document.getElementById(container_id) == null) {
        ...
        const p_attempts = document.createElement('p')
  ...
  initialize () {
    ...
    for (const event_name of ['mouseup', 'touchend']) {
    ...
    for (const element_id of [ELEMENTS.MODE_SETTING_AUTOPLAY_ONLY, ELEMENTS.MODE_SETTING_NONE]) {
